...
export const KPICards: FC<KPICardsProps> = ({
  ...
  const displayDealers = selectedMonthData ? selectedMonthData.activeDealers : metrics.activeDealersCount;

  // Consistent Fulfillment Rate calculation
  const scopedTransactions = selectedMonthData ? selectedMonthData.transactions : metrics.filteredTransactions;
  const completedCount = scopedTransactions.filter(t => t.status === 'Completed').length;
  const displayFulfillment = scopedTransactions.length > 0
    ? Number(((completedCount / scopedTransactions.length) * 100).toFixed(1))
    : 0;
  ...
  const cards = [
    {
      ...
      change: selectedPeriod ? null : metrics.revenueChange,
      ...
      description: selectedPeriod
        ? `${selectedPeriod} माह में डीलर नेटवर्क द्वारा कुल मांग (${scopedTransactions.length} चालान)`
    ...
    },
    {
      ...
      change: selectedPeriod ? null : metrics.unitsChange,
      ...
      description: selectedPeriod
        ? `${selectedPeriod} में भेजे गए कुल पार्ट्स (${displayDealers} सक्रिय डीलर केंद्र)`
        : `1,746 कैटलॉग SKU में से ${metrics.catalogCount} चालू पुर्जे`,
    },
    {
      title: "चालान पूर्ति दर / Fulfillment Rate",
      value: `${displayFulfillment}%`,
      change: selectedPeriod ? null : metrics.fulfillmentChange,
      periodLabel: selectedPeriod ? `${selectedPeriod} Fulfillment` : "Q1 Fulfillment",
      icon: CheckCircle2,
      iconBg: "bg-[#fef3c7] text-[#b45309] dark:bg-[#0e3d22] dark:text-[#f0b429]",
      description: selectedPeriod
        ? `${selectedPeriod} में ${displayDealers} सक्रिय डीलरों को 100% फिटमेंट-जांच पूर्ण चालान`
        : `जीरो-इन्वेंट्री मॉडल में फिटमेंट-जांच पूर्ण ऑर्डर्स (${displayDealers} सक्रिय डीलर)`,
    },
  ];
  ...
  return (
    <div className="space-y-3">
      ...
      {selectedPeriod && (
        <div className="flex flex-wrap items-center justify-between gap-2.5 rounded-xl bg-[#e9f4ed] px-4 py-2.5 border border-[#dbe7de] dark:bg-[#14532d] dark:border-[#1b7a43]">
          ...
          {onResetPeriod && (
            <button
              ...
              className="inline-flex min-h-[44px] items-center gap-2 rounded-xl bg-white px-4 py-2 text-xs sm:text-sm font-bold text-[#1b7a43] shadow-xs hover:bg-[#1b7a43] hover:text-white dark:bg-[#0e3d22] dark:text-[#f6f8f5] dark:hover:bg-[#1b7a43] transition-colors cursor-pointer"
            >
      ...
      <section aria-label="Executive Overview KPIs" className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {cards.map((card) => {
          const hasChange = card.change !== null && card.change !== undefined && card.change !== 0;
